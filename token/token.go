...
//provided private key which is associated with the application.
func GenerateToken(subject, scope, clientID, applicationName, privateKey string) (string, error) {
...
func GenerateCode(subject, scope string, clientID string, privateKey string) (string, error) {
	t := jwt.New(jwt.GetSigningMethod("RS256"))
...
func GenerateKeyExtractionFunctionForJTWFlow(issuer, publicKey string) jwt.Keyfunc {
	return func(token *jwt.Token) (interface{}, error) {
		//Check the signing method
		if _, ok := token.Method.(*jwt.SigningMethodRSA); !ok {
			return nil, fmt.Errorf("Unexpected signing method: %v", token.Header["alg"])
